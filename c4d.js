...
var Settings = {};
try{Settings = require('./config.json');}catch(e){return console.log("Missing Configuration file");}
...
CryptoBot.prototype.sendEmail = function(email_message)
	{
		...
		return server.send(message,function(err, message){if(err){console.log("Error Sending Email:",err);}});
	}
			...
			try
				{
					...
					req.open("POST",Settings.Slack.hook);
					req.onload=()=>{console.log("Slack Message Sent:"+req.responseText);}
					req.onerror=(e)=>{console.log(e);}
					return req.send(JSON.stringify(parameters));
				}
							  ...
							  if(err) { return (console.log("Unable to connect to the database:",err)); }
							  else
								{
									db.createCollection('bittrexBalance',{strict:true},function(err,collection)
										{ 
											if(err){console.log("Bittrex Balance Collection Already Created");}
											else
												{
													db.collection('bittrexBalance').createIndex( { "Time": 1 }, { unique: true } )
												}
										});	
									DB.balance = db.collection('bittrexBalance');		
									db.createCollection('bittrexHistory',{strict:true},function(err,collection)
										{ 
											if(err){console.log("Bittrex History Collection Already Created");}
											else
												{
									...
									db.createCollection('bittrexTrade',{strict:true},function(err,collection)
										{ 
											if(err){console.log("Bittrex Trade Collection Already Created");}
											else
												{
								...
								try
									{
										...
										if(message.command === "bittrex_db")
											{
												return this.retrieveDB(message.db)
												.then((que)=>
													{
...
CryptoBot.prototype.bittrexAPI = function(path,options)
	{
		return new Promise((resolve,reject) => 
			{	
				...
				if(options && options.length > 0){postData = postData + options}
					        ...
					        response.on("end",function() 
						        {
									...
									catch(e){console.log("Bittrex API Error:",e);return reject(e);}		
						            return resolve(parsed);
						        });
					        response.on('error',(e)=>{console.log("Bittrex API Error:",e);return reject(e);})
						}
			    );
			   return req.end();
			});
...
CryptoBot.prototype.bittrexAccount = function()
	{
		return new Promise((resolve,reject)=>
			{
				return this.bittrexAPI("account/getbalances",null)
				.then((list)=>
					{
						if(!list){console.log("Error getting Bittrex account info");return reject(false);}
						list.map((asset)=>{return this.balance[asset.Currency.toLowerCase()] = asset.Available;});
				...
				.catch((e)=>{console.log("Error getting bittrex balance:",e);return reject(false);});
			});
	}
...
CryptoBot.prototype.bittrexArbitrage = function(pair1,pair2,pair3)
	{
		return new Promise((resolve,reject)=>
			{
				return Promise.all([this.bittrexDepth(pair1),this.bittrexDepth(pair2),this.bittrexDepth(pair3)])
				.then((values)=>
					{
						var a;
						var b;
										...
										if(Transactions[u2+'_amount'] > Transactions[u2] && Transactions[b3+'_amount'] > Transactions[b3] && Transactions[e1+'_amount'] > Transactions[e1])
											{
												if(Transactions[e1+'_status'] && Transactions[b3+'_status'] && Transactions[u2+'_status'])
													{
														...
														return this.bittrexTrade("sell",pair3,Transactions[e1],c)
														.then(()=>
															{
																this.bittrexTrade("buy",pair2,Number(Transactions[b3].toFixed(8)),b)
																.catch((e)=>
																	{
														...
														.then(()=>
															{
																this.bittrexTrade("buy",pair1,Transactions[_e1].toFixed(8),a)
																.catch((e)=>
																	{
												...
												else{console.log("Current balances not enough to send transfer on Liqui",Transactions);}
											}else{console.log("Conditions Not Met:",Transactions);}	
									}
							}
						if(percentage > 100)
							{
								...
								if(percentage > 100.75 && percentage < 101.9 && Transactions[u2+'_amount'] > Transactions[u2] && Transactions[b3+'_amount'] > Transactions[b3] && Transactions[e1+'_amount'] > Transactions[e1+'_amount'])
									{
										if(Transactions[e1+'_status'] && Transactions[b3+'_status'] && Transactions[u2+'_status'])
											{
												...
												console.log("Starting Trades");
												try{this.slackMessage(message);}catch(e){console.log(e);}
												return this.bittrexTrade("sell",pair2,Transactions[b3],b)
												.then(()=>
													{
														this.bittrexTrade("buy",pair3,Transactions[e1].toFixed(8),c)
														.catch((e)=>
															{
												...
												.then(()=>
													{
														this.bittrexTrade("sell",pair1,Transactions[e1].toFixed(8),a)
														.catch((e)=>
															{
										...
										else{console.log("Current balances not enough to perform trades",Transactions);}
									}
								else{console.log("Conditions Not Met",Transactions);}									
							}
		...
		.then((orders)=>
			{
				...
				if(!orders || orders === null){this.balance.orders = false;z= -1;}
				else if(JSON.stringify(orders) === "{}" || orders.length < 1){z=0;}
				else{z =1;}
						...
						.catch((e)=>
							{
								...
								return this.bittrexAccount().then(()=>{this.bittrexPoll();}).catch((e)=>{this.bittrexPoll();});
							})	
					}
			})
		.catch((e)=>{console.log("Error:",e);return setTimeout(()=>{this.bittrexCompleteArbitrage();},this.rate * 3);});		
	}	
...
CryptoBot.prototype.bittrexDepth = function(pair)
	{
		return new Promise((resolve,reject) => 
			{	
			    https.get({host: "bittrex.com",path: "/api/v1.1/public/getorderbook?market="+pair+"&type=both"},
			    (response)=> 
				    {
				        var body = '';
				        ...
				        response.on('end',()=> 
					        {	
								try{
						            ...
						            //console.log(parsed)
						            if(!parsed || !parsed.success){return reject("Error:"+body);}
				    ...
				    }).on('error',(e)=>{console.log(e);return reject(e);});
			})
	}
...
CryptoBot.prototype.bittrexGetOrders = function()
	{
		return new Promise((resolve,reject) => 
			{	
				return this.bittrexAPI("market/getopenorders",null)
					.then((orders)=>
						{
		...
		var _add = function (percentage)
					{
						...
						if(_history.DB && _history.DB.history)
							{
								return _history.DB.history.insert({"Time":date,"Percent":percentage}, {w:1}, 
									function(err, result) 
										{
											if(err){console.log("Error adding percentage to DB:",err);}
											else{console.log("Percentage added to DB");}
											return;
										});	
							}
					}
		...
		var _saveDB = function(type,doc)
			{
				try
					{
						if(!_history.DB[type]){return console.log("Database Does Not Exist");}
						return _history.DB[type].insert(doc, {w:1}, 
							function(err, result) 
								{
									if(err){console.log("Error adding "+type+" to DB:",err);}
									else{console.log(type+" added to DB");}
									return;
								});	
					}
...
CryptoBot.prototype.retrieveDB = function(type)
	{
		return new Promise((resolve,reject) => 
			{	
				try
					{
						if(!this.DB || !this.DB[type]){this.DB = this.database();}
						return this.DB[type].find({}).toArray(function(err, items)
							{
								if(err){console.log(err);return resolve(e);}
								else{return resolve(items);}
							});									
					}
				catch(e){console.log(e);return resolve(e)}
			})
	}	
CryptoBot.prototype.saveDB = function(type,doc)
	{
		try
			{
				if(!this.Settings.MongoDB.connect){return;}
				if(!this.DB || !this.DB[type]){this.DB = this.database();}
				return this.DB[type].insert(doc, {w:1}, 
					function(err, result) 
						{
							if(err){console.log("Error adding "+type+" to DB:",err);}
							else{console.log(type+" added to DB");}
...
CryptoBot.prototype.bittrexTrade = function(type,pair,quantity,rate)
	{
		return new Promise((resolve,reject) => 
			{	
				return this.bittrexAPI("market/"+type+"limit","&rate="+rate+"&market="+pair+"&quantity="+quantity)
				.then((result)=>{return resolve(result);})
				.then((e)=>{return reject(e);});
			})
	}
...
function main()
	{
		...
		return bot.setupWebsocket()
		.then(()=>
			{
				return bot.bittrexAccount()
				.then(()=>{return bot.bittrexPoll();})
				.catch(e=>{return bot.bittrexPoll();});
			});
	}
main();
